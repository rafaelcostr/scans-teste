 ...
 * Suporta: arbitrum, base, bsc, polygon (stable explícito — ver AGGREGATOR_POLYGON_STABLE).
 *
 * Env: ENABLE_AGGREGATORS=1, ONEINCH_API_KEY, ZEROX_API_KEY (um ou ambos)
 * Polygon: AGGREGATOR_POLYGON_STABLE (endereço USDC/USDC.e a usar), AGGREGATOR_POLYGON_STABLE_DECIMALS (default 6)
 */
...
/**
 * @returns {{ base: string, stable: string, stableDecimals: number } | null}
 */
function aggregatorTokensForChain(chain) {
  const c = chain.toLowerCase();
  if (c === "polygon") {
    try {
      const raw = process.env.AGGREGATOR_POLYGON_STABLE;
      const stable =
        raw && String(raw).trim()
          ? getAddress(String(raw).trim())
          : getAddress("0x3c499c542cef5e3811e1192ce70d8cc03e5e3342");
      const decRaw = process.env.AGGREGATOR_POLYGON_STABLE_DECIMALS;
      const stableDecimals =
        decRaw != null && String(decRaw).trim() !== ""
          ? parseInt(String(decRaw), 10)
          : 6;
      const base = getAddress("0x7ceb23fd6bc0add59e62ac25578270cff1b9f619");
      return {
        base,
        stable,
        stableDecimals:
          Number.isFinite(stableDecimals) && stableDecimals > 0 && stableDecimals <= 36
            ? stableDecimals
            : 6
      };
    } catch {
...
async function tryAggregatorRoundtrip(
  ...
  const t = aggregatorTokensForChain(chain);
  if (!t) {
    return {
      ok: false,
      code: "SKIP",
      reason: `Rede ${chain} sem mapa agregador ou AGGREGATOR_POLYGON_STABLE inválido.`
    };
  }
  ...
  const analysis = {
    ...
    note:
      chain === "polygon"
        ? "Round-trip agregador em Polygon com stable fixada por AGGREGATOR_POLYGON_STABLE (default USDC nativo). Verifique alinhamento com os pools DexScreener."
